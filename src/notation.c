...
static struct option long_options[] = {
    {"long-algebric",0,0,'a'},
    {"short-algebric",0,0,'s'},
    {"input-language",1,0,'f'},
    {"output-language",1,0,'t'},
    {"output-file",1,0,'o'},
    {"show-after",1,0,'c'},
    {"end-after",1,0,'e'},
    {"board-only",0,0,'b'},
  ...
  int options_index = 0;
  test = getopt_long(argc,argv,"asf:t:o:c:e:bd:ihv",long_options,&options_index);
  optindcopy = optind;
  if (test == -1)
    break;
    //(void) strcpy (cp,argv[narg+1]); // do a for loop that loops through c and finds where it begins
  switch (test) {
      case 'f' : /* from langage */
  if  (optarg == 0)
...
if ((infile = fopen (argv[optindcopy],"r")) == NULL){
